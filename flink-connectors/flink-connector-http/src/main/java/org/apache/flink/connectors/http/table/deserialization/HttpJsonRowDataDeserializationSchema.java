...
import static org.apache.flink.connectors.http.table.deserialization.TimeFormats.*;
 ...
 */
@Internal
public class HttpJsonRowDataDeserializationSchema implements DeserializationSchema<List<RowData>> {
    ...
    private final TypeInformation<List<RowData>> resultTypeInfo;
    ...
    /** The data filed name of response. */
    private final List<String> responseDataFields;

    public HttpJsonRowDataDeserializationSchema(
            RowType rowType,
            TypeInformation<List<RowData>> resultTypeInfo,
            boolean failOnMissingField,
            boolean ignoreParseErrors,
            TimestampFormat timestampFormat,
            List<String> responseDataFields) {
        ...
        this.responseDataFields = responseDataFields;
    }

    @Override
    public List<RowData> deserialize(byte[] message) throws IOException {
        try {
            JsonNode root = objectMapper.readTree(message);
            if (CollectionUtils.isNotEmpty(responseDataFields)) {
                for (String field : responseDataFields) {
                    root = root.get(field);
                }
                if (root == null) {
                    throw new RuntimeException(
                            "Not found data from response by keys: " + responseDataFields);
                }
            }
    ...
    @Override
    public boolean isEndOfStream(List<RowData> nextElement) {
        return false;
    }

    @Override
    public TypeInformation<List<RowData>> getProducedType() {
    ...
    public boolean equals(Object o) {
        ...
        HttpJsonRowDataDeserializationSchema that = (HttpJsonRowDataDeserializationSchema) o;
    ...
    private Object convertField(
            DeserializationRuntimeConverter fieldConverter, String fieldName, JsonNode field) {
        if (field == null) {
