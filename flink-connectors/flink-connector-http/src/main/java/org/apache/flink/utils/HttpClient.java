	...
	public Tuple2<Integer, String> request(HttpRequestBase requestBase) throws IOException {
		...
		try (CloseableHttpClient httpClient = getHttpClient()) {
			RequestConfig requestConfig = RequestConfig.custom()
				.setSocketTimeout(
					socketTimeout == null ? HttpConstants.DEFAULT_SOCKET_TIMEOUT : socketTimeout)
				.setConnectTimeout(
					connectTimeout == null ? HttpConstants.DEFAULT_CONNECT_TIMEOUT : connectTimeout)
				.setConnectionRequestTimeout(
					requestTimeout == null ? HttpConstants.DEFAULT_REQUEST_TIMEOUT : requestTimeout)
				.build();
			...
			if (response.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
				LOG.error(
					"Failed to do the request, with status code:{}",
